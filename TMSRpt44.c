...
//  FIELD11=PayTime,NUMERIC,7,2,7,0,
//  FIELD12=BusNumber,VARCHAR,16,0,16,0,
//  FIELD13=ActualDriver,VARCHAR,51,0,51,0,
//
//
//  Dispatch sheet report
//

BOOL FAR TMSRPT44(TMSRPTPassedDataDef *pPassedData)
{
...
//
  ...
  strcat(tempString, "\\TMSRPT44.txt");
...
//
//  See if he just wants reassigned work
//
  MessageBeep(MB_ICONQUESTION);
  if(MessageBox(hWndMain, "Do you want to see just the reassigned runs?", TMS, MB_ICONQUESTION | MB_YESNO) == IDYES)
  {
    bJustReassigned = TRUE;
  }
  else
  {
    bJustReassigned = FALSE;
  }
...
//
//  Check on just reassigned
//
    if(bJustReassigned && RData[nI].replacementDRIVERSrecordID == NO_RECORD)
    {
      continue;
    }
//
...
//      time = RUNSVIEW[nJ].runOnTime - RUNSVIEW[nJ].reportTime;
      time = RUNSVIEW[nJ].runOnTime - COST.TRAVEL[nJ].startTravelTime - COST.PIECECOST[nJ].reportTime;
      if(RData[nI].DRIVERSrecordID != NO_RECORD)
      {
        sprintf(outputString, "%ld\t%s\t\"%s - %s, %s\"\t%ld\t",
              time, RParamData.szDate, badgeNumber, lastName,
              firstName, RData[nI].runNumber);
      }
      else
      {
        sprintf(outputString, "%ld\t%s\t\t%ld\t",
              time, RParamData.szDate, RData[nI].runNumber);
      }
//
//  Block number
//
...
//  Off time
//
      if(COST.TRAVEL[nJ].endTravelTime > 0)
...
//  Total pay (in tenths)
//
      hour = COST.TOTAL.payTime / 3600;
      min = (COST.TOTAL.payTime % 3600) / 60;
//
//  Force the decimal to these constraints:
//
      dmin = 0;
      for(nK = 2; nK < 60; nK += 6)
      {
      ...
      sprintf(tempString, "%d.%d\t", hour, dmin);
      strcat(outputString, tempString);
//
//  Bus number
//
...
//
        ...
        {
          sprintf(outputString, "%ld\t%s\t\"%s - %s, %s\"\t%ld\t\t",
                xTime[0], RParamData.szDate, badgeNumber[0], lastName[0],
                firstName[0], RData[nI].runNumber);
        }
        else
        {
          sprintf(outputString, "%ld\t%s\t\t%ld\t\t",
