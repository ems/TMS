...
//    FIELD11=PiecePay,VARCHAR,8,0,8,0,
//    FIELD12=Text,VARCHAR,256,0,256,0,
//
#include "TMSHeader.h"
#include "cistms.h"
#include <math.h>
...
BOOL FAR TMSRPT71(TMSRPTPassedDataDef *pPassedData)
{
...
//
  ...
  strcat(tempString, "\\TMSRPT71.TXT");
...
//
    for(nJ = 0; nJ < numPieces; nJ++)
    {
      piecePay[nJ] = RUNSVIEW[nJ].reportTime + RUNSVIEW[nJ].startOfPieceTravel + 
            RUNSVIEW[nJ].platformTime + RUNSVIEW[nJ].endOfPieceTravel + RUNSVIEW[nJ].turninTime;
    }
//
...
//
    for(nJ = 0; nJ < numPieces; nJ++)
    {
      numComments = 0;
      numSigncodes = 0;
      strcpy(szPiecePay, chhmm(piecePay[nJ]));
      strcpy(szReportTime, Tchar(pieceReportTime[nJ]));
      strcpy(szEndTime, Tchar(pieceEndTime[nJ]));
//
...
//
              sprintf(tempString, "%ld\t\"%s\"\t\"%s\"\t%ld\t%ld\t%d\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\r\n",
                    ...
                    szRuntype, szReportTime, szPayTime, szEndTime, szPiecePay);
              ...
              sprintf(tempString, "%ld\t\"%s\"\t\"%s\"\t%ld\t%ld\t%d\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\r\n",
                    seq++, divisionName, serviceName, RUNS.runNumber, RUNS.pieceNumber, numPieces,
                    szRuntype, szReportTime, szPayTime, szEndTime, szPiecePay, szarString);
              WriteFile(hOutputFile, (LPCVOID *)tempString, strlen(tempString), &dwBytesWritten, NULL);
//
//  Any extraboard?
//
...
//                    sprintf(szarString, "Piece %d: Start Travel Instructions:\n", RUNS.pieceNumber);
                    ...
                    while(ptr != 0)
                    {
                      sprintf(tempString, "%ld\t\"%s\"\t\"%s\"\t%ld\t%ld\t%d\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\r\n",
                            seq++, divisionName, serviceName, RUNS.runNumber, RUNS.pieceNumber, numPieces,
                            szRuntype, szReportTime, szPayTime, szEndTime, szPiecePay, ptr);
                      WriteFile(hOutputFile, (LPCVOID *)tempString, strlen(tempString), &dwBytesWritten, NULL);
              ...
              if(!((pTRIPSAVE->flags & TRIPSAVE_FLAG_PULLOUT) && TRIPS.standard.POGNODESrecordID == RUNS.start.NODESrecordID))
              {
                ...
                if(reliefRunNumber != NO_RECORD)
                {
                  sprintf(tempString, "%ld\t\"%s\"\t\"%s\"\t%ld\t%ld\t%d\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\r\n",
                        ...
                        szRuntype, szReportTime, szPayTime, szEndTime, szPiecePay);
                  WriteFile(hOutputFile, (LPCVOID *)tempString, strlen(tempString), &dwBytesWritten, NULL);
                  sprintf(tempString, "%ld\t\"%s\"\t\"%s\"\t%ld\t%ld\t%d\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\t\"This piece of work relieves run %ld\"\r\n",
                        seq++, divisionName, serviceName, RUNS.runNumber, RUNS.pieceNumber, numPieces,
                        szRuntype, szReportTime, szPayTime, szEndTime, szPiecePay, reliefRunNumber);
                  WriteFile(hOutputFile, (LPCVOID *)tempString, strlen(tempString), &dwBytesWritten, NULL);
                }
              }
            }
...
//
            sprintf(tempString, "%ld\t\"%s\"\t\"%s\"\t%ld\t%ld\t%d\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\t\"ROUTE %s ",
                  seq++, divisionName, serviceName, RUNS.runNumber, RUNS.pieceNumber, numPieces,
                  szRuntype, szReportTime, szPayTime, szEndTime, szPiecePay, pTRIPSAVE->routeNumber);
...
//
            if(bIncludeTripNumbers)
              strcpy(szarString, "Sign   Trip #   ");
            else
              strcpy(szarString, "Sign   ");
            ...
            strcat(szarString, "Next     Next  Comment");
            sprintf(tempString, "%ld\t\"%s\"\t\"%s\"\t%ld\t%ld\t%d\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\r\n",
                  seq++, divisionName, serviceName, RUNS.runNumber, RUNS.pieceNumber, numPieces,
                  szRuntype, szReportTime, szPayTime, szEndTime, szPiecePay, szarString);
            WriteFile(hOutputFile, (LPCVOID *)tempString, strlen(tempString), &dwBytesWritten, NULL);
...
//
            ...
            strcat(szarString, "Route    Trip  Code");
            sprintf(tempString, "%ld\t\"%s\"\t\"%s\"\t%ld\t%ld\t%d\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\r\n",
                  seq++, divisionName, serviceName, RUNS.runNumber, RUNS.pieceNumber, numPieces,
                  szRuntype, szReportTime, szPayTime, szEndTime, szPiecePay, szarString);
            WriteFile(hOutputFile, (LPCVOID *)tempString, strlen(tempString), &dwBytesWritten, NULL);
...
//
          if(TRIPS.standard.PIGNODESrecordID != NO_RECORD)
          {
            ...
            strncpy(garageAbbr, NODES.abbrName, NODES_ABBRNAME_LENGTH);
            trim(garageAbbr, NODES_ABBRNAME_LENGTH);
            sprintf(nextRoute, "P/I-%s@", garageAbbr);
            GCTData.fromPATTERNNAMESrecordID = TRIPS.PATTERNNAMESrecordID;
            GCTData.toPATTERNNAMESrecordID = TRIPS.PATTERNNAMESrecordID;
            GCTData.fromROUTESrecordID = TRIPS.standard.RGRPROUTESrecordID;
            GCTData.fromSERVICESrecordID = TRIPS.standard.SGRPSERVICESrecordID;
...
//
          ...
          if(timeFormat == PREFERENCES_MILITARY)
          {
            if(bIncludeTripNumbers)
            {
              sprintf(tempString, "%ld\t\"%s\"\t\"%s\"\t%ld\t%ld\t%d\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\t\"%6s  %6s",
                    seq++, divisionName, serviceName, RUNS.runNumber, RUNS.pieceNumber, numPieces,
                    szRuntype, szReportTime, szPayTime, szEndTime, szPiecePay, signCode, szTripNumber);
            }
            else
            {
              sprintf(tempString, "%ld\t\"%s\"\t\"%s\"\t%ld\t%ld\t%d\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\t\"%6s",
                    seq++, divisionName, serviceName, RUNS.runNumber, RUNS.pieceNumber, numPieces,
                    szRuntype, szReportTime, szPayTime, szEndTime, szPiecePay, signCode);
            }
          }
          else
          {
...
//  Next Trip
//
          sprintf(szarString, " %8s%6s%8s\"\r\n", nextRoute, szPITime, commentCode);
          strcat(tempString, szarString);
          WriteFile(hOutputFile, (LPCVOID *)tempString, strlen(tempString), &dwBytesWritten, NULL);
...
//
      ...
      if(reliefRunNumber != NO_RECORD)
      {
        ...
        {
          sprintf(tempString,
                "%ld\t\"%s\"\t\"%s\"\t%ld\t%ld\t%d\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\t\"This piece of work is relieved by run %ld\"\r\n",
                seq++, divisionName, serviceName, RUNS.runNumber, RUNS.pieceNumber, numPieces,
                szRuntype, szReportTime, szPayTime, szEndTime, szPiecePay, reliefRunNumber);
        }
        else
        {
          sprintf(tempString,
                "%ld\t\"%s\"\t\"%s\"\t%ld\t%ld\t%d\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\t\"This piece of work is relieved by run %ld, which starts on route %s\"\r\n",
                seq++, divisionName, serviceName, RUNS.runNumber, RUNS.pieceNumber, numPieces,
                szRuntype, szReportTime, szPayTime, szEndTime, szPiecePay, reliefRunNumber, szParkForRoute);
        }
        WriteFile(hOutputFile, (LPCVOID *)tempString, strlen(tempString), &dwBytesWritten, NULL);
      }
//
...
//
      ...
      WriteFile(hOutputFile, (LPCVOID *)tempString, strlen(tempString), &dwBytesWritten, NULL);
//
//  Spit out the comments for this piece
//
      if(numComments > 0)
      {
        sprintf(tempString, "%ld\t\"%s\"\t\"%s\"\t%ld\t%ld\t%d\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\r\n",
              ...
              szRuntype, szReportTime, szPayTime, szEndTime, szPiecePay);
        WriteFile(hOutputFile, (LPCVOID *)tempString, strlen(tempString), &dwBytesWritten, NULL);
        sprintf(tempString, "%ld\t\"%s\"\t\"%s\"\t%ld\t%ld\t%d\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\t\"Comments:\"\r\n",
              seq++, divisionName, serviceName, RUNS.runNumber, RUNS.pieceNumber, numPieces,
              szRuntype, szReportTime, szPayTime, szEndTime, szPiecePay);
        WriteFile(hOutputFile, (LPCVOID *)tempString, strlen(tempString), &dwBytesWritten, NULL);
...
//
//  Spit out the sign codes for this piece
//
      if(numSigncodes > 0 && bIncludeSignCodes)
      {
        sprintf(tempString, "%ld\t\"%s\"\t\"%s\"\t%ld\t%ld\t%d\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\r\n",
              ...
              szRuntype, szReportTime, szPayTime, szEndTime, szPiecePay);
        WriteFile(hOutputFile, (LPCVOID *)tempString, strlen(tempString), &dwBytesWritten, NULL);
        sprintf(tempString, "%ld\t\"%s\"\t\"%s\"\t%ld\t%ld\t%d\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\t\"Signcodes:\"\r\n",
              seq++, divisionName, serviceName, RUNS.runNumber, RUNS.pieceNumber, numPieces,
              szRuntype, szReportTime, szPayTime, szEndTime, szPiecePay);
        WriteFile(hOutputFile, (LPCVOID *)tempString, strlen(tempString), &dwBytesWritten, NULL);
...
//
    if(bGotRunComment)
    {
      sprintf(tempString, "%ld\t\"%s\"\t\"%s\"\t%ld\t%ld\t%d\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\r\n",
            ...
            szRuntype, szReportTime, szPayTime, szEndTime, szPiecePay);
      WriteFile(hOutputFile, (LPCVOID *)tempString, strlen(tempString), &dwBytesWritten, NULL);
      sprintf(tempString, "%ld\t\"%s\"\t\"%s\"\t%ld\t%ld\t%d\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\t\"Run comment:\"\r\n",
            seq++, divisionName, serviceName, RUNS.runNumber, RUNS.pieceNumber, numPieces,
            szRuntype, szReportTime, szPayTime, szEndTime, szPiecePay);
      WriteFile(hOutputFile, (LPCVOID *)tempString, strlen(tempString), &dwBytesWritten, NULL);
      sprintf(tempString, "%ld\t\"%s\"\t\"%s\"\t%ld\t%ld\t%d\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\t\"%s\"\r\n",
