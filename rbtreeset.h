...
private:
	...
	struct Node
	{
		Node( const Key &dt ) : Data(dt) {}
		Node *left;         /* left child */
		Node *right;        /* right child */
		Node *parent;       /* parent */
		NodeColor color;    /* node color (black, red) */
		Key Data;			/* data stored in node */
	};
...
public:
	...
	public:
		...
		int operator!=( const iterator &i ) const { return n != i.n; }
		const Key &operator*() const { return n->Data; }
		Key &operator*() { return n->Data; }

		iterator( const RBTreeSet *aTree, Node *aN ) : tree(aTree), n(aN) {}

		const RBTreeSet	*tree;
		Node	*n;
	};
	...
	RBTreeSet() { init(); }
	RBTreeSet( const RBTreeSet &t ) { init(); operator=(t); }
	~RBTreeSet() { clear(); }
	...
	RBTreeSet &operator=( const RBTreeSet &t )
	{
		clear();
		if( !t.empty() )
		{
	...
	insert_pair insert(const Key &Data)
	{
	...
	iterator find( const Key &key ) const
	{
		...
		*******************************/
		...
		while(current != NIL)
			if( CompLT(key, current->Data) )
				current = current->left;
			else if( CompLT(current->Data, key) )
				current = current->right;
			else
				break;
	...
	iterator findPredecessor( const Key &key ) const
	{
		...
		return parent == NIL ? end() :
			CompLT(parent->Data, key) ? iterator(this, parent) :
	...
	iterator findSuccessor( const Key &key ) const
	{
		...
		return parent == NIL ? end() :
			CompLT(key, parent->Data) ? iterator(this, parent) :
...
#ifdef MMenableMemoryManager
template <class Key, class Compare>
MMallocator	*RBTreeSet< Key, Compare >::allocator = NULL;
#endif

template <class Key, class Compare>
RBTreeSet< Key, Compare >::Node *RBTreeSet<Key, Compare>::NIL = NULL;
