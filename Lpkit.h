...
#define get_x(lp, i) 	((lp)->best_solution[(lp)->rows+(i)])
#define get_obj_val(lp) ((lp)->best_solution[0])
...
#define MALLOC(ptr, nr, type) if((ptr = (type *) \
  ...
  __LINE__, __FILE__); exit(FAIL); }
#else
#define MALLOC(ptr, nr, type) if((ptr = (type *) malloc((size_t)((nr) * sizeof(type)))) == 0) {fprintf(stderr, "malloc failed on line %d of file %s\n", __LINE__, __FILE__); exit(FAIL); }
#endif

#define REALLOC(ptr, nr, type) if((ptr = (type *) realloc(ptr,(size_t)(nr)*sizeof(type))) == 0) {fprintf(stderr, "realloc failed on line %d of file %s\n", __LINE__, __FILE__); exit(FAIL); }
