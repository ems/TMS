...
double LatOrigin(int ZoneLetter)
{
  int Letters[] =   {'C', 'D', 'E', 'F', 'G', 'H', 'J', 'K', 'L', 'M', 'N', 'P', 'Q', 'R', 'S', 'T', 'U', 'V', 'W', 'X', 'Z'};
  double Origin[] = {-80, -72, -64, -56, -48, -40, -32, -24, -16,  -8,   0,   8,  16,  24,  32,  40,  48,  56,  64,  72,   0};
  int  nI;

  for(nI = 0; nI < 20; nI++)
  {
    if(ZoneLetter == Letters[nI])
    {
      return(Origin[nI]);
    }
  }
...
//  Nonearth Units "m" Bounds (363509, 4895890)(401582, 4925883)
...
//	*Lat = LatOrigin((int)*ZoneLetter) + *Lat * rad2deg;
	*Lat = *Lat * rad2deg;
